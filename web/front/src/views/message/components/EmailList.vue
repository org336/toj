...
const cardHeight = computed(() => window.innerHeight - 105);
    ...
    <div class="email-list">
      <transition-group name="fade">
        <div v-for="email in filterdEmailList" :key="email.id">
          <div class="email-item d-flex align-center pa-5" @click="">
            <div>
              <v-checkbox-btn
                v-model="email.read"
                color="primary"
            ...
            <v-btn icon>
              <v-icon icon="star" :color="email.starred ? 'yellow' : 'grey'"></v-icon>
            </v-btn>
            <v-btn icon @click="emailStore.deleteEmailById(email.id)">
              <v-icon icon="delete" color="grey"></v-icon>
            </v-btn>
          </div>
        </div>
      </transition-group>
