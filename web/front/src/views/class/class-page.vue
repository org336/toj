...
import { UserService, ClassService } from "@/utils/api";
  ...
  ClassService.removeStudentsFromClass(id, params)
    .then((res) => {
      if (res.code === 200) {
        // 更新班级中的学生列表
        getManyByClass();
        ElMessage.success("学生已成功移除出班级");
      } else {
        ElMessage.error("移除学生出班级失败");
      }
    })
    .finally(() => {
      // 清空选中的学生
